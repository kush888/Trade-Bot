...
class CustomEnv:
    ...
    def reset(self, env_steps_size = 0):
        ...
        self.start_price = self.df.iloc[self.start_step]["Close"]
        self.end_price = self.df.iloc[self.end_step]["Close"]
        for i in reversed(range(self.lookback_window_size)):
            ...
            for column in self.columns:
                arr.append(self.df_normalized.loc[current_step, column])
            self.market_history.append(arr)
...
#         else:
#             return 0
        reward = (self.net_worth-1000)/10 - ((close - self.start_price)/self.start_price)*100
        return reward
...
def test_agent(test_df, test_df_nomalized, visualize=True, test_episodes=10, folder="", name="", comment="", Show_reward=False, Show_indicators=False):
    ...
    print("average {} episodes agent net_worth: {}, orders: {}".format(test_episodes, average_net_worth/test_episodes, average_orders/test_episodes))
    print("No profit episodes: {}".format(no_profit_episodes))
    print ("Return Percentage: ", (average_net_worth/test_episodes-1000)/10)
    # save test results to test_results.txt file
    with open("test_results.txt", "a+") as results:
        current_date = datetime.now().strftime('%Y-%m-%d %H:%M')
        results.write(f'{current_date}, {name}, test episodes:{test_episodes}')
...
if __name__ == "__main__":            
    ...
    fileName = "./6 years 1h interval BTCUSDT data"
    ...
    cryptoReturns = ((test_df.iloc[-1]["Close"] - test_df.iloc[0]["Close"]) / test_df.iloc[0]["Close"])*100
    ...
    test_agent(test_df,test_df_nomalized, visualize=False, folder="2022_02_04_02_36_Crypto_trader", name="1742.21_Crypto_trader", test_episodes=1  )
    print ("Crypto Return Percentage: ", cryptoReturns)
    # single processing training
#     agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.00001, epochs=5, optimizer=Adam, batch_size = 32, model="CNN", depth=13)
#     train_env = CustomEnv(df=train_df, df_normalized=train_df_nomalized, lookback_window_size=lookback_window_size)
#     train_agent(train_env, agent, visualize=False, train_episodes=32, training_batch_size=500)
...
#     print ("START")
#     time = datetime.now()
#     train_multiprocessing(CustomEnv, agent, train_df, train_df_nomalized, num_worker = 12, training_batch_size=500, visualize=False, EPISODES=200000, folder="2022_02_04_02_36_Crypto_trader", name="1742.21_Crypto_trader")
